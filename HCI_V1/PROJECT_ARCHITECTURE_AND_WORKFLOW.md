...
> **Current production app:** `HCI_2.1` (not the legacy `Hci_1` paths below).  
> **Official run/deploy docs:** `RUN_GUIDE.md`, `DEPLOY.md`.

## HCI_2.1 dual entry points (current)

Node webhook integration is **additive**. The Google Drive / Web UI flow is unchanged. Both share the same business logic:

```text
Google Drive / UI routes          Node Frontend
        │                              │
        ▼                              ▼
 Existing /api/*              POST /api/generate-ifc-webhook
        │                              │
        └──────────────┬───────────────┘
                       ▼
                 Shared core
...
| Entry | Isolated storage | Touches train dataset? |
|-------|------------------|------------------------|
| Drive / UI | `gdrive_dataset/images_*`, `labels/train`, `output/` | Yes (by design) |
| Webhook | `gdrive_dataset/webhook_uploads/{job_id}/` | **No** |

Backward compatible: Drive, Auto Label, Corrections, Training, UI IFC unchanged; Node is another adapter only.

---

This document originally explained the project under `D:\HCI_interor\Hci_1`:
...
File paths below that still say `Hci_1` map conceptually to the same layout under **`HCI_2.1`** for the live app.
