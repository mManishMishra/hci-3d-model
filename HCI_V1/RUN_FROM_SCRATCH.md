...
## Node webhook (additive — does not replace the UI)

After the server is running on port 8000, the Node doorway is:

```text
POST http://127.0.0.1:8000/api/generate-ifc-webhook
```

Multipart fields: `image`, `callback_url` (required), optional `meters_per_pixel`.

Immediate JSON: `{ "accepted": true, "job_id": "...", "status": "processing" }`.  
IFC is delivered later via POST to `callback_url` (`success` + `ifc_file`, or `success=false` + `error`).

Job files live under:

```text
D:\HCI_interor\gdrive_dataset\webhook_uploads\{job_id}\
  input\  labels\  output\  work\
```

This path is **isolated** from `images_raw` / `labels/train`. Full details: `RUN_GUIDE.md` §11, `DEPLOY.md`.
