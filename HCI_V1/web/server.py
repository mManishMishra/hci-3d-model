...
import os, sys, glob, shutil, threading, json, base64, traceback, asyncio, uuid, logging
...
from logic.scale_calibration import save_scale, load_scale, resolve_mpp, DEFAULT_MPP
...
def _resolve_image_for_basename(basename: str) -> Path | None:
    """Prefer train image, then raw image."""
    for folder in (DATASET_DIR / "images" / "train", DATASET_DIR / "images_raw"):
...
def generate_ifc_geometry(basename: str, body: dict = None):
    ...
    """
    body = body or {}
    lbl = DATASET_DIR / "labels" / "train" / (basename + ".txt")
    if not lbl.exists():
        return JSONResponse(
            {"error": f"No saved labels for {basename}. Auto-label / correct / save first."},
            status_code=404,
        )
    img_path = _resolve_image_for_basename(basename)
    if img_path is None:
        return JSONResponse({"error": f"No image found for {basename}"}, status_code=404)

    if body.get("meters_per_pixel") is not None:
        mpp = float(body["meters_per_pixel"])
        mpp_source = "request"
    else:
        mpp, mpp_source = resolve_mpp(DATASET_DIR, basename, DEFAULT_MPP)

    IFC_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    out_ifc = IFC_OUTPUT_DIR / f"{basename}.ifc"
    work_dir = IFC_OUTPUT_DIR / "_debug" / basename
    ...
    result["download_url"] = f"/api/ifc/file/{basename}"
    _push(
        f"✅ IFC generated: {out_ifc.name}  walls={result.get('ifc_walls')} "
...
def download_ifc_file(basename: str):
    ...
    path = IFC_OUTPUT_DIR / f"{basename}.ifc"
    if not path.exists():
        return JSONResponse(
...
# ── Node frontend webhook (additive; isolated from Drive / UI dataset) ────────
# Reuses generate_labels() + generate_full_ifc(). Does NOT call _autolabel_worker,
# does NOT write images_raw / images/train / labels/train / marked / runs / dataset.yaml,
# and does NOT touch _analysis / _push / UI SSE.
_webhook_log = logging.getLogger("hci21.webhook")
if not _webhook_log.handlers:
    _handler = logging.StreamHandler(sys.stderr)
    _handler.setFormatter(logging.Formatter("[hci21.webhook] %(message)s"))
    _webhook_log.addHandler(_handler)
    _webhook_log.setLevel(logging.INFO)
    _webhook_log.propagate = False

WEBHOOK_UPLOADS_DIR = DATASET_DIR / "webhook_uploads"


def _webhook_write_label_file(label_path: Path, label_lines: list) -> None:
    """Write YOLO-seg lines to a job-local label file (webhook only)."""
...
def _webhook_post_callback(
    ...
    *,
    ...
    """POST multipart result to Node callback_url. Never raises to caller."""
    import requests
    ...
    file_handle = None
    try:
        _webhook_log.info("Webhook Job %s: Posting callback → %s", job_id, callback_url)
        if success and ifc_path is not None and ifc_path.is_file():
            file_handle = open(ifc_path, "rb")
            files = {
                "ifc_file": (ifc_path.name, file_handle, "application/octet-stream"),
            }
        resp = requests.post(callback_url, data=data, files=files, timeout=120)
        if 200 <= resp.status_code < 300:
            _webhook_log.info("Webhook Job %s: Callback success (HTTP %s)", job_id, resp.status_code)
        else:
            _webhook_log.error(
                "Webhook Job %s: Callback failed (HTTP %s) body=%s",
                job_id,
                resp.status_code,
                (resp.text or "")[:500],
            )
    except Exception as exc:
        _webhook_log.error("Webhook Job %s: Callback failed: %s", job_id, exc)
    finally:
...
def _webhook_generate_ifc_worker(
    job_id: str,
    image_path: str,
    label_path: str,
    output_ifc: str,
    ...
    """
    Background worker: generate_labels → job-local .txt → generate_full_ifc → callback.
    Isolated workspace only; no UI / Drive / training side effects.
    """
    from logic.yolo_inference import ModelNotFoundError

    img_p = Path(image_path)
    lbl_p = Path(label_path)
    out_p = Path(output_ifc)
    work_p = Path(work_dir)

    try:
        _webhook_log.info("Webhook Job %s: Image saved (%s)", job_id, img_p.name)
        _webhook_log.info("Webhook Job %s: Running YOLO (generate_labels)", job_id)
        label_lines, img, labelled = generate_labels(str(img_p))
        if img is None:
            reason = (labelled or {}).get("_skip_reason", "image unreadable")
            raise RuntimeError(f"YOLO / image load failed: {reason}")
        if not label_lines:
            reason = (labelled or {}).get("_skip_reason", "no labels")
            raise RuntimeError(f"YOLO produced no labels: {reason}")

        _webhook_write_label_file(lbl_p, label_lines)
        _webhook_log.info(
            "Webhook Job %s: YOLO complete (%d polygons) → %s",
            job_id,
            len(label_lines),
            lbl_p.name,
        )

        _webhook_log.info("Webhook Job %s: Generating IFC (generate_full_ifc)", job_id)
        ...
        if not out_p.is_file():
            raise RuntimeError("generate_full_ifc returned but IFC file missing")

        _webhook_log.info(
            "Webhook Job %s: IFC generated walls=%s doors=%s windows=%s → %s",
            job_id,
        ...
        _webhook_post_callback(
            callback_url, job_id=job_id, success=True, ifc_path=out_p
        )
    except ModelNotFoundError as exc:
        _webhook_log.error("Webhook Job %s: Model not found: %s", job_id, exc)
        _webhook_post_callback(
            callback_url, job_id=job_id, success=False, error=str(exc)
        )
    except Exception as exc:
        _webhook_log.error(
            "Webhook Job %s: Failed: %s\n%s", job_id, exc, traceback.format_exc()
        )
        _webhook_post_callback(
            callback_url, job_id=job_id, success=False, error=str(exc)
        )
...
@app.post("/api/generate-ifc-webhook")
async def generate_ifc_webhook(
    ...
    """
    Node frontend doorway: accept image + callback_url, run existing
    generate_labels → generate_full_ifc in an isolated job folder, then
    POST IFC (or error) to callback_url. Does not touch Drive/UI dataset paths.
    """
    cb = (callback_url or "").strip()
    if not cb or not (cb.startswith("http://") or cb.startswith("https://")):
        return JSONResponse(
    ...
    if ext not in IMG_EXTS:
        return JSONResponse(
            {"error": f"unsupported image type '{ext}'", "allowed": sorted(IMG_EXTS)},
            status_code=400,
        )

    job_id = uuid.uuid4().hex
    job_root = WEBHOOK_UPLOADS_DIR / job_id
    input_dir = job_root / "input"
    labels_dir = job_root / "labels"
    output_dir = job_root / "output"
    work_dir = job_root / "work"
    for d in (input_dir, labels_dir, output_dir, work_dir):
        d.mkdir(parents=True, exist_ok=True)

    # Preserve extension; stem used for label/ifc names (job_id isolates collisions).
    ...
    output_ifc = output_dir / f"{stem}.ifc"
    ...
    try:
        ...
        _webhook_log.error("Webhook Job %s: Failed to save image: %s", job_id, exc)
        return JSONResponse({"error": f"failed to save image: {exc}"}, status_code=500)

    mpp = float(meters_per_pixel) if meters_per_pixel is not None else float(DEFAULT_MPP)
    if mpp <= 0:
        return JSONResponse(
            {"error": "meters_per_pixel must be > 0"},
            status_code=400,
        )

    _webhook_log.info("Webhook Job %s accepted (file=%s mpp=%s)", job_id, safe_name, mpp)

    background_tasks.add_task(
        _webhook_generate_ifc_worker,
        job_id,
        str(image_path),
        str(label_path),
        str(output_ifc),
