...
class DatasetCleaner:
    ...
    def create_splits(self, image_paths: list[Path]) -> dict[str, list[Path]]:
        """Assign images to train/val/test using configured ratios."""
        raise NotImplementedError(
            "Train/val/test splits are not implemented yet. "
            "Use DatasetCleaner.run() for deduplication only."
        )
    ...
    def write_split_manifest(self, splits: dict[str, list[Path]], manifest_path: Path) -> None:
        ...
        raise NotImplementedError(
            "Split manifest writing is not implemented yet."
        )
